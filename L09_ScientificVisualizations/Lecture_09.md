...
### Splitting Criteria
The algorithm chooses splits to maximize purity, using metrics like:
- **Gini Impurity:** $I_G(p) = 1 - \sum_{i=1}^{J} p_i^2$ (measure of how often a randomly chosen element would be misclassified).
- **Information Gain (Entropy):** $H(T) = -\sum_{i=1}^{J} p_i \log_2 p_i$ (measure of disorder).

### Regression with Trees (CART)
Trees can also be used for regression. Instead of predicting a class, leaves predict a continuous value (e.g., the mean of the target values in that leaf). Splits are chosen to minimize a loss function like **Mean Squared Error (MSE)** or **Mean Absolute Error (MAE)**.
